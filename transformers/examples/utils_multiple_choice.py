...
class MyProcessor(DataProcessor):
    ...
    def _create_examples(self, lines: List[List[str]], type: str):
        """Creates examples for the training and dev sets."""
        if (type == "train" or type == "dev") and lines[0][-1] != 'AnswerRightEnding':
            raise ValueError(
                "For training and validation, the input file must contain a label column."
            )
        if type == "test":
            examples = [
                InputExample(
                    example_id=line[0],
                    question=line[4],
                    contexts = [line[1]+line[2]+line[3], line[1]+line[2]+line[3]],
